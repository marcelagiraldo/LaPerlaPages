...
export default {
    ...
    methods: {
        ...
        async deletePayroll(payrollId) {
            try {
                const response = await axios.delete(`http://localhost:8001/payroll/payrolls/${payrollId}`);
        ...
        <table>
            ...
            <tbody>
                <tr v-for="payroll in payrolls" :key="payroll.id">
                    ...
                    <td><a href="" @click="deletePayroll(payroll.id)"><svg xmlns="http://www.w3.org/2000/svg" width="30px" height="30px" fill="currentColor" class="bi bi-trash3-fill" viewBox="0 0 16 16">
